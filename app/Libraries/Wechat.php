...
class Wechat
{
	...
	const CARD_MEMBERCARD_ACTIVATE_GETURL        = '/card/membercard/activate/geturl?';      //获取开卡插件参数
	const CARD_MEMBERCARD_ACTIVATE_INFO        = '/card/membercard/activatetempinfo/get?';      //获取跳转型开卡用户输入参数
	const CARD_MEMBERCARD_USERINFO        = '/card/membercard/userinfo/get?';      //会员卡信息
	const CARD_MEMBERCARD_ACTIVATE        = '/card/membercard/activate?';      //激活会员卡
	const CARD_MEMBERCARD_UNACTIVATE        = '/card/membercard/unactivate?';      //解绑会员卡
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    */
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$authname = 'ikea_wechat_access_token'.$appid;
		if ($rs = Cache::get($authname))  {
			$this->access_token = $rs;
			return $rs;
		}

		$url = env('WOAAP_WWW_URL') . '/api/accesstoken?appid=' . $appid.'&token='. env('WOAAP_TOKEN');
		...
		if ($result)
		{
			...
			Cache::put($authname,$this->access_token,3);
			return $this->access_token;
		}
	...
	public function resetJsTicket($appid=''){
		if (!$appid) $appid = $this->appid;
		$this->jsapi_ticket = '';
		$authname = 'ikea_wechat_jsapi_ticket'.$appid;
		$this->removeCache($authname);
		return true;
	}
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
		...
		*/
		//定制
		$url = env('WOAAP_WWW_URL') . '/api/jsticket?appid=' . $this->appid.'&token='. env('WOAAP_TOKEN');
		...
		if ($result)
		{
			...
			$this->jsapi_ticket = $json['js_ticket'];
			$expire = isset($json['expires_in']) ? intval($json['expires_in'])-3600 : 3600;
     ...
     * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getApiTicket($appid=''){
        if (!$this->access_token && !$this->checkAuth()) return false;
        if (!$appid) $appid = $this->appid;

        $authname = 'ikea_wechat_card_jsapi_ticket'.$appid;
        ...
        //定制
        $url = env('WOAAP_WWW_URL') . '/api/apiticket?appid=' . $this->appid .'&token='. env('WOAAP_TOKEN');
        ...
        if ($result)
        {
            ...
            Cache::put($authname, $json['api_ticket'], 60);
            return $json['api_ticket'];
        }
        return false;
    }
	 ...
	 * @param int $type 0:临时二维码；1:永久二维码(此时expire参数无效)；2:永久二维码(此时expire参数无效) 3 : 临时二维码 字符串参数
		...
		*/
        ...
        if($type == 2 || $type == 1){
            if(is_string($scene_id)){
                ...
                $actionInfo = ['scene'=>['scene_str'=>$scene_id]];
            }else{
                $actionName = 'QR_LIMIT_SCENE';
                $actionInfo = ['scene'=>['scene_id'=>$scene_id]];
            }
        }else{
            if(is_string($scene_id)){
	...
	*/
	private function getAc(){
		$authname = 'ikea_wechat_ackey'.env('WECHAT_MP_APPID');
		if ($rs = Cache::get($authname))  {
			//$this->ackey = $rs;
			return $rs;
		}
		
		$url = env('WOAAP_WWW_URL') . '/api/accesstoken?appid=' . env('WECHAT_MP_APPID');
		$result = file_get_contents($url);
		$access_token = json_decode($result)->access_token;
		if ($access_token)
		{
			Cache::put($authname,$access_token,3);
		}
		return $access_token;
	}
...
// $url = env('WOAAP_WWW_URL') . '/api/accesstoken?appid=' . env('WECHAT_MP_APPID');
// $result = file_get_contents($url);
$access_token = $this->getAc(); //json_decode($result)->access_token;
...
$result = $this->http_get($url);
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     */
    public function getUserInfoMini($openid){
        /*
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
        */
        $url = env('WOAAP_WWW_URL_MINI') . '/api/accesstoken?appid=' . env('WECHAT_MP_APPID');
        $result = file_get_contents($url);
        $access_token = $this->getAc(); //json_decode($result)->access_token;
        $url = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$access_token.'&openid='.$openid.'&lang=zh_CN';
	...
	public function sendCustomMessage($data){
		...
		$access_token = $this->getAc();
$url = self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$access_token;
		$result = $this->http_post($url,self::json_encode($data));
		Log::info('Jaakko@SendCustom: ' . $result);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 发送客服消息
     * @param array $data(json 后的)
     * @return boolean|array
     */
    public function sendCustomMessageTmp($data){
        //if (!$this->access_token && !$this->checkAuth()) return false;
        //       $this->access_token = 'bNSu16dO04U7-UzNzR8s067ZalRtlB7XaYnJC04BdCPuBc4Wv_zigSFQMIFBZJkfmxZ9ORfwCyS2BmJNiTYAP-zDSv2v9LwaMTBnOcktioM';
        //$result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$this->access_token,$data);
		$access_token = $this->getAc();
...
$url = self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$access_token;
	...
	public function getOauthRedirect($callback,$state='',$scope='snsapi_userinfo'){
		$url = env('WOAAP_API_URL') . '/api/oauth2-authorize?';
     ...
     * @return string
     */
    public function getOauthRedirectMini($callback,$state='',$scope='snsapi_userinfo'){
        $url = env('WOAAP_API_URL_MINI') . '/api/oauth2-authorize?';
        return $url.'appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
    }
	...
	public function getOauthAccessToken(){
		...
		$url = env('WOAAP_API_IP') . '/api/oauth2-accesstoken?';
		$result = $this->http_get($url.'appid='.$this->appid.'&token='. env('WOAAP_TOKEN').'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 通过code获取Access Token (Mini)
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getOauthAccessTokenMini(){
        $code = isset($_GET['code'])?$_GET['code']:'';
        if (!$code) return false;

        $url = env('WOAAP_API_IP_MINI') . '/api/oauth2-accesstoken?';
        $result = $this->http_get($url.'appid='.$this->appid.'&token='. env('WOAAP_TOKEN').'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
        if ($result)
        {
            $json = json_decode($result,true);
	...
	public function sendTemplateMessage($data){
        $access_token = $this->getAc();
		//$result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SEND_URL.'access_token='.$access_token,self::json_encode($data));
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$access_token,self::json_encode($data));
		if($result){
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	 ...
	 * @return boolean
	 */
	public function unavailableCard($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_NAVAILABLE. 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function consumeCardCode($code,$card_id='') {
		...
		if ($result) {
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg  = $json['errmsg'];
                $this->writeVerifyLog($json['errcode'], $json['errmsg']);
				return false;
			}
			return $json;
		}
	...
	public function checkAckey($appid=''){
	    $appkey = env('WOAAP_APPKEY');
        if (!$appid) {
            $appid = $this->appid;
        }

        $authname = 'ikea_wechat_ackey'.$appid;
        if ($rs = Cache::get($authname))  {
            $this->ackey = $rs;
            return $rs;
        }

        $url = env('WOAAP_API_IP') . '/api/ackey?appid=' . $appid . '&appkey='. $appkey;
        $result = $this->http_get($url);
        if ($result)
        {
            $json = json_decode($result,true);
            ...
            $this->ackey = $json['ackey'];

            Cache::put($authname,$this->ackey,3);
            return $this->ackey;
        }
	...
	public function decryptCardCode($encrypt_code) {
		...
		if (!$this->checkAuth($appid)) return false;
		// if (!$this->ackey && !$this->checkAckey()) return false;
		Log::info('Jaakko@DecryptToken:' . $this->access_token );
        $url = self::API_BASE_URL_PREFIX . self::CARD_CODE_DECRYPT . 'access_token='.$this->access_token;
	    $result = $this->http_post($url,self::json_encode($data));
	    Log::info('Jaakko@Decrypt:' . $result);
  		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	 ...
	 * }
	 */
	public function checkCardCode($code, $cardid = '', $check_consume = true)
    {
		$data['code'] = $code;
	 ...
	 * }
	 */
	public function getCardList($openid,$card_id='',$filter=true) {
		if(empty($openid)){
			$this->errCode = '400';
			$this->errMsg  = 'openid为空.';
			return false;
		}
		$data = [];
		$data['openid'] = $openid;
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_LIST. 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
			...
			//过滤会员卡	env:  MEMBER_CARD_ID & MINIPROGRAM_CARD_ID
			if($filter && !empty($json['card_list'])){
				foreach($json['card_list'] as $k=>$card){
					if(in_array($card['card_id'],[env('MEMBER_CARD_ID'),env('MINIPROGRAM_CARD_ID'), env('MINIPROGRAM_CENTER_CARD_ID')])){
						unset($json['card_list'][$k]);
					}
				}
			}
     ...
     * @return boolean
     */
    public function unactivateMemberCard($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_UNACTIVATE . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 获取开卡插件参数
     * @param string $data { "card_id" : "pbLatji6o5g7hJh8Otuvux4y1ty0", "outer_str" : "123" }
     * @return boolean
     */
    public function getUrlMemberCard($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_ACTIVATE_GETURL. 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 获取跳转型开卡用户输入参数
     * @param string $data { "activate_ticket" : "pbLatji6o5g7hJh8Otuvux4y1ty0" }
     * @return boolean
     */
    public function getMemberCardActivateInfo($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_ACTIVATE_INFO. 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
	 ...
	 * @return boolean
	 */
	public function viewMemberCard($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_USERINFO. 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
